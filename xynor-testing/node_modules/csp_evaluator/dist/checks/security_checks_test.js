...
describe('Test security checks', () => {
    ...
    it('CheckScriptUnsafeInlineWithMalformedNonce', () => {
        const test = 'script-src \'unsafe-inline\' \'nonce-invalid$\';';
        const parsedCsp = (new parser_1.CspParser(test)).csp;
        const effectiveCsp = parsedCsp.getEffectiveCsp(csp_1.Version.CSP3);
        const violations = securityChecks.checkScriptUnsafeInline(effectiveCsp);
        expect(violations.length).toBe(1);
        expect(violations[0].severity).toBe(finding_1.Severity.HIGH);
    });
    it('CheckScriptUnsafeInlineWithMalformedHash', () => {
        const test = 'script-src \'unsafe-inline\' \'sha256-invalid$\';';
        const parsedCsp = (new parser_1.CspParser(test)).csp;
        const effectiveCsp = parsedCsp.getEffectiveCsp(csp_1.Version.CSP3);
        const violations = securityChecks.checkScriptUnsafeInline(effectiveCsp);
        expect(violations.length).toBe(1);
