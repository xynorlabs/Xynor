    ...
    <section className="relative py-32">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="relative">
          <div className="absolute left-1/2 top-0 hidden h-full w-px bg-gradient-to-b from-transparent via-white/20 to-transparent lg:block" />
          {stages.map((stage, i) => (
            <motion.div
              key={stage.label}
              ...
              className="group relative mb-8 flex flex-col items-center gap-6 lg:flex-row"
            >
              <div className={`flex-1 ${i % 2 === 0 ? "lg:text-right" : "lg:order-2 lg:text-left"}`}>
                <div className="glass inline-block rounded-2xl p-6 transition-all group-hover:border-white/20">
                  <div className="font-space text-sm font-medium text-white/50">0{i + 1}</div>
                  <div className="mt-1 font-space text-2xl font-bold">{stage.label}</div>
                  <div className="mt-1 text-sm text-white/50">{stage.desc}</div>
                </div>
              </div>
